...
while ~isempty(openList)
    ...
    for i = 1:size(neighbors,1)
        ...
        if nx < 1 || ny < 1 || nx > map.GridSize(2) || ny > map.GridSize(1)
            continue;
        end
        if visited(ny, nx) || checkOccupancy(map, [nx ny])
            continue;
        end
...
for k = 1:maxNodes
    ...
    if any(newNode < 0) || any(newNode > 100), continue; end
    ...
    for i = nearby'
        ...
        if newCost < costs(i) && isMotionValid(map, nodes(i,:), nodes(newIdx,:), 0.5)
            parents(i) = newIdx;
            costs(i)   = newCost;
        end
    end

    if norm(newNode-goal) < goalThreshold && isMotionValid(map,newNode,goal,0.5)
        goalIdxRRT = newIdx+1;
...
%% 5. Final Visualization (LEGEND FIXED)
figure('Name','A* vs RRT* Comparison','Position',[100 100 1800 700]);
...
title('A* Pathfinding');
plot(expandedNodes(:,1), expandedNodes(:,2), 'k.');
plot(pathAStar(:,1), pathAStar(:,2), 'r-', 'LineWidth',2);
plot(start(1),start(2),'go','LineWidth',2);
plot(goal(1),goal(2),'bx','LineWidth',2);
legend({'Visited','A* Path','Start','Goal'}, ...
    'Location','southoutside','Orientation','horizontal');

% RRT*
nexttile;
show(map); hold on; axis([0 100 0 100]);
title('RRT* Pathfinding');
for i = 2:size(nodes,1)
    plot([nodes(i,1),nodes(parents(i),1)], ...
         [nodes(i,2),nodes(parents(i),2)], 'Color',[1 0.6 0]);
end
plot(pathRRT(:,1), pathRRT(:,2), 'r-', 'LineWidth',2);
plot(start(1),start(2),'go','LineWidth',2);
plot(goal(1),goal(2),'bx','LineWidth',2);
legend({'RRT* Tree','RRT* Path','Start','Goal'}, ...
    'Location','southoutside','Orientation','horizontal');
