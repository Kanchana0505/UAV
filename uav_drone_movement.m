...
%% RETURN PATH (add return journey)
return_path = flipud(path_apf);
full_path = [path_apf; return_path];
...
latCenter = mean(lat); lonCenter = mean(lon);
campos(g1, latCenter, lonCenter, 500);
campos(g2, latCenter, lonCenter, 500);
campos(g3, latCenter, lonCenter, 500);

plotCommon = @(g) [
    geoplot3(g, lat(1), lon(1), 0, 'o', 'MarkerSize',10, 'LineWidth',2, 'Color','g', 'HeightReference','terrain');
    geoplot3(g, lat(2), lon(2), 0, 'o', 'MarkerSize',10, 'LineWidth',2, 'Color','r', 'HeightReference','terrain');
...
plotCommon(g3);
geoplot3(g3, lat_smooth, lon_smooth, alt_smooth, '-', 'Color',[1 0.5 0], 'LineWidth',2.5, 'HeightReference','terrain');
geoplot3(g3, lat_apf, lon_apf, alt_apf, '-', 'Color','c', 'LineWidth',2, 'HeightReference','terrain');

%% ANIMATED PATH ON 2D REAL MAP (GEOAXES)
figure('Name','Animated Path on Real Map','Color','w');
gx = geoaxes;
geobasemap(gx, 'satellite');
hold(gx, 'on');

% Start & End markers
geoscatter(gx, lat(1), lon(1), 80, 'g', 'filled');
text(lat(1), lon(1), '  START', 'Color', 'g', 'FontSize', 12);
geoscatter(gx, lat(2), lon(2), 80, 'm', 'filled');
text(lat(2), lon(2), '  GOAL', 'Color', 'm', 'FontSize', 12);

% Animate
N = length(lat_full);
for i = 1:N
    droneSize = 40 + 0.05 * alt_full(i);
    color = [1, 0.5 * sin(alt_full(i)/70), 1 - 0.5 * cos(alt_full(i)/90)];
    drone = geoscatter(gx, lat_full(i), lon_full(i), droneSize, color, 'filled');
    geoplot(gx, lat_full(1:i), lon_full(1:i), 'c-', 'LineWidth', 1.4);
    pause(0.01);
    if i < N, delete(drone); end
end
